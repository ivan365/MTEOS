...
local appEngine = {
    ...
    startApp = function(self, appPath, mode)
        mode = mode or "-d"
    ...
    startAppAdvanced = function (self, appPath, flagString)
        local flags = {}
        local lines = {}
        ...
        if not port then error("No free port") end
                        ...
                        if not fn then
                            ...
                            if app.mode == "-s" then
                                env = {
                                    ...
                                    reopenApp = function() appEngine:startAppAdvanced("/1.lua", "-d -pf") end,
                                    appMode = app.mode
                                }
                            else
                                env = {}
                                ...
                                env.reopenApp = function() appEngine:startAppAdvanced("/1.lua", "-d -pf") end
                            end
                            setfenv(fn, env)
...
-------EVENT MANAGER-------
local EMTEeventEngine = {
    EventDataBase = {},

    registerEvent = function(self, victim, sender_data, data, filter, time)
        table.insert(self.EventDataBase[victim], {data = data, sender = sender_data, filter = filter, time = time}) 
    end,

    readEvent = function(self, port, mode) --mode 1 = read and remove, mode 0 = read only, 
        if mode == 0 then
            local data = self.EventDataBase[port][#self.EventDataBase[port]]
            return data
        else
            local data = self.EventDataBase[port][1]
...
appEngine:startAppAdvanced("/1.lua", "-d -pf")
appEngine:showAppByPort(12)
